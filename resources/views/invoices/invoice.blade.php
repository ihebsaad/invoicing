	...
	<div class="container">
		<div style="width:100%;">
			...
			<div style="width:50%;float:left;height:100px">
				...
				{{ $invoice->customer()->first()->civility2 }} {{ $invoice->customer()->first()->name2 }} {{ $invoice->customer()->first()->lastname2 }}<br>			
			...
			@if($invoice->modalite!='')
				...
				@if($invoice->modalite!='Chèque')
					<table class="financement">
						<tr><td>Montant Financé :</td><td style="padding-right:20px">{{$invoice->montant_finance}}</td><td>Montant mensuel<br>de l'assurance :</td><td>{{$invoice->montant_assurance}}</td></tr>
						<tr><td>Report 1ère échéance :</td><td style="padding-right:20px">{{$invoice->report_echeance}}</td><td>Taux nominal :</td><td>{{$invoice->taux_nominal}}</td></tr>
						<tr><td>Nombre de mensualités :</td><td style="padding-right:20px">{{$invoice->mensualites}}</td><td>TAEG :</td><td>{{$invoice->taeg}}</td></tr>
						<tr><td>Montant mensuel<br>sans assurance :</td><td style="padding-right:20px">{{$invoice->montant_mensuel}}</td><td>Solde de la pose :</td><td>{{$invoice->pose}}</td></tr>
					</table>
				@endif
			@endif
			</div>
